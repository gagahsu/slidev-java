...
- **基礎字元符號是地基**：`\d`（數字）、`\w`（字母數字底線）、`\s`（空白）、`.`（任意單一字元）各有分工，大寫版本代表「非」
- **`[]` 自訂字元集合，`[^]` 在開頭加 `^` 表示否定**；`&&` 可在中括號內做交集
- **特殊符號要用 `\\` 跳脫**，且 Java 字串裡的反斜線要寫兩個（regex 的 `\d` 在 Java 字串要寫成 `"\\d"`）
- **`|` 是 OR 運算**，可以把多種格式串在同一個表達式裡
- **量次修飾詞控制重複次數**：`?`（0或1次）、`*`（0或多次）、`+`（1或多次）、`{n}`／`{n,}`／`{n,m}` 可精確指定次數
- **`()` 分組讓量次修飾詞套用在整個群組上**，也是擷取比對內容的基礎
- **String 類別的四個 regex 方法**：`matches()` 比對整個字串、`split()` 用 regex 分割、`replaceFirst()`／`replaceAll()` 取代符合的部分

<!--
我們把這一章整理成七個重點。

第一，基礎字元符號是正規表達式的地基：\d 比對數字、\w 比對字母數字底線、\s 比對空白、. 比對任意單一字元（換行除外），這些符號的大寫版本剛好代表相反的意思，例如 \D 就是非數字。

第二，[] 讓我們自訂一個字元集合，只要比對到集合裡任一個字元就算符合；如果在中括號最開頭加上 ^，就變成否定，代表「不包含」這些字元；還可以用 && 做交集，縮小比對範圍。

第三，正規表達式裡有特殊意義的符號（像是句點、括號、星號），如果要比對它們的字面值，必須加反斜線跳脫；而且因為 Java 字串本身也需要跳脫，反斜線要寫兩個，這是初學者最容易搞混的雙重跳脫問題。

第四，管道符號 | 就是 OR 的意思，可以把多種格式寫在同一個表達式裡，例如同時驗證手機格式和市話格式。

第五，量次修飾詞控制「前面的符號或群組要出現幾次」：? 是 0 或 1 次、* 是 0 或多次、+ 是 1 或多次，{n}、{n,}、{n,m} 則可以更精確地指定次數或範圍。

第六，小括號 () 用來把多個字元打包成一個群組，讓量次修飾詞可以套用在整個群組上，這也是之後 Matcher.group() 能夠擷取內容的基礎。

第七，String 類別提供的四個方法是最終落地的工具：matches() 判斷整個字串是否符合、split() 用正規表達式當分隔符、replaceFirst() 和 replaceAll() 分別取代第一個或所有符合的部分。

這七點加起來，就是這一章正規表達式的完整地圖。如果對反向引用、具名分組、環視斷言、Pattern/Matcher 這些更進階的寫法有興趣，歡迎參考本章的進階自學內容，下一章我們要繼續往下走了！
-->
