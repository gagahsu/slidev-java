...
# 🎬 AI 協作時刻：看不懂的 regex 找 AI 翻譯

正規表達式最痛苦的地方，往往不是「自己寫」，而是「看懂別人寫的」。遇到一長串符號看不懂時，直接請 AI 逐段拆解：

**要用的 Prompt：**

> 請幫我逐段拆解這個正規表達式，告訴我每一部分分別在比對什麼，
> 並舉一個「符合」跟一個「不符合」的字串範例：
> `^1[3-9]\d{9}$`

<div class="mt-4 p-3 bg-blue-50 border-l-4 border-blue-400 text-gray-700 text-sm text-left">
💡 <b>實務上的用法：</b> 之後在專案裡看到同事寫的複雜正規表達式，與其自己一個符號一個符號硬猜，直接貼給 AI 拆解會快很多，這才是進職場後真正會用到的技能。
</div>

<!--
【操作提示】
現場貼上這個 prompt，讓 AI 拆解出 `^`、`1`、`[3-9]`、`\d{9}`、`$` 各自的意思，並舉例說明這是常見的中國大陸手機號碼格式，藉機複習今天教過的字元類別和量次符號。

【收斂一句話】
寫 regex 靠練習，看懂別人的 regex 靠 AI 幫你逐段拆解——這是進入職場後最實用的合作方式。
-->
---
layout: default
---
