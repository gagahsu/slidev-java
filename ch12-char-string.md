...
<!--
【開場白】
各位未來的架構師們，大家好！我是你們今天的講師，一個在 Code 裡打滾十年，在脫口秀舞台上被趕下來十次的男人。今天我們要聊 Java 裡最基礎、但也最容易讓你在半夜三點抱著螢幕哭的主題：字元（Character）和字串（String）。

【為什麼要學這個？】
寫程式如果不處理文字，那就像去熱炒店不點啤酒一樣空虛。不管是帳號密碼，還是你現在看到的簡報，通通是文字。學好這章，你就掌握了 Java 的呼吸法，不然你寫的 Code 就像沒熱身的馬拉松選手，跑不到一公里就 OOM（記憶體溢位）了。
...
<!--
【核心說明】
今天的行程很紮實。我們會從小到大的處理：先看單個字（Character），再看一串字（String）。我還會帶大家去參觀 Java 的「神祕地下室」——字串池（String Pool），這可是面試官最愛問的題目。最後會教大家怎麼用「變形金剛」StringBuilder 來拼字串，保證你的程式效能從腳踏車變跑車。
...
<!--
【核心說明】
當你拿到一個字，你想知道它是誰。它是數字？還是字母？Character 類別就是你的身分證查驗官。

【逐步解說】
isDigit 就是看它是不是 0 到 9。注意喔，這是「字元版」的數字，如果你傳 10 進去，它會報錯，因為它是單身狗，一次只能處理一個字。
驚喜的是 isLetter，Java 的 Unicode 支援非常好，你餵它一個「炭」字，它也會點頭說是 True。

⚠️ 學生常見誤解：
記得！這些方法是 static 的。你不能寫 myChar.isDigit()，你要寫 Character.isDigit(myChar)。這就像是你不能自己幫自己發身分證，得去戶政事務所（類別名）一樣。
-->
...
【逐步解說】
isWhitespace 就像是那個最愛乾淨的房東，只要是會產生空白的東西（Tab、換行），它通通都算。
但 isSpaceChar 就比較挑嘴，它只認標準的「空格」。這在處理使用者亂打的資料（比如在名字後面按了一堆 Tab）時非常重要。
...
<!--
【核心說明】
轉換大小寫沒什麼好說的，就是 toLowerCase 和 toUpperCase。

【逐步解說】
我想強調的是那個 \u3000（全型空白）。這真的是中文開發者的惡夢。有時候使用者在網頁表單按了一個全型空白，你肉眼看不出來，但資料庫就是搜尋不到，這時候 isSpaceChar 就是你的救命恩人。
...
【生活化比喻】
這就像是劇本裡的「動作標記」。當你看到 \n，不是要你唸出來，是要你「換行」。如果你在台上大喊「反斜線 n」，導演會把你踢出去的。這在 Java 裡叫 Escape Character，讓那些打不出來的符號也能乖乖聽話。
...
【逐步解說】
宣告字串就像是在變魔術。直接用引號是最常見的。
但 String 有很多「隱藏版玩法」，比如你可以把字元陣列丟進去。

💼 業界實務：
不要隨便用 new String("xxx")。這在業界就像是去超市買礦泉水還要自備水瓶一樣，多此一舉還佔空間。除非你真的有什麼奇怪的需求，不然直接給它雙引號就好了。
-->
...
<!--
【核心說明】
Java 終於開竅了！以前要寫一段長長的文字（像是 SQL 語法），你要在那邊 + "\n" + 弄半天，寫到懷疑人生。

【逐步解說】
現在有了 """ 三個引號，你直接在裡面換行、排版，Java 都會幫你保持原樣。這簡直是工程師的福音，代碼看起來乾淨得像剛裝潢好的樣品屋。
...
<!--
【核心說明】
這招叫「文字縮骨功」。你想把整段話往右移或往左縮，一個 indent 搞定。正數往右，負數往左。這在做報表輸出或是排版時，能讓你少寫很多 for 迴圈去補空格。
...
<!--
【核心說明】
各位，重點來了！請把你的視線從旁邊的珍珠奶茶移過來。這就是著名的「字串池」。

【生活化比喻】
Java 是個勤儉持家的好男人。如果你跟它要一個「鬼殺隊」，它會去倉庫看有沒有，有的話就給你同一把鑰匙（位址）。所以 s1 == s2 是 True。
為什麼要這樣？因為字串太常用了，如果每個人要一個都蓋一間新房子，記憶體會比你的錢包還快乾涸。
-->
...
<!--
【核心說明】
但如果你硬要用 new，Java 就會說：「好啦，大爺您有錢，我就幫你另外蓋一間一模一樣的。」這時候雖然內容一樣，但鑰匙（位址）就不一樣了。

【逐步帶著看】
因此，s1 == s3 是 false，因為它們在記憶體的不同角落。但如果你比內容（用 equals），那當然還是 true。

⚠️ 學生常見誤解：
這就是為什麼我們常說「比較字串內容要用 equals」，因為 you 永遠不知道這兩個字串是不是來自同一個「倉庫」。
-->
...
<!--
【核心說明】
字串是 Immutable 的，這點很重要，要考（敲黑板）。當你把 str 從「炭治郎」改成「火之神神樂」時，原本的「炭治郎」並沒有消失，它還在那裡，只是 str 這根手指不指它了。

【逐步帶著看】
這就像是你換女朋友，原本那個還在世上，懷念歸懷念，只是你不再牽她的手了。（喂，這比喻會不會太危險？）
...
<!--
【核心說明】
length() 就是看長度，沒什麼好說的。但 isEmpty 和 isBlank 的差別一定要搞懂。

【逐步解說】
isEmpty 就像是錢包裡連一塊錢都沒有；isBlank 就像是錢包裡塞滿了發票但還是沒錢（只有空白）。
...
【程式世界怎麼用】
如果你要做「不區分大小寫」的搜尋，記得先把大家通通變成小寫再比。這就像是大家都穿上制服（LowerCase），這樣就不會因為有人穿得太花俏而認不出來了。
-->
...
<!--
【核心說明】
這頁請大家截圖存檔！如果你不想在半夜兩點被系統噴出來的 NullPointerException 吵醒，就一定要學會這招。

【生活化比喻】
null 就像是你的前任傳來的訊息：看起來有東西，但點進去什麼都沒有。如果你試圖對它做任何事，你的程式就會崩潰（就像你的心一樣）。
...
<!--
【核心說明】
我最推 Objects.requireNonNullElse。它的意思就是：「如果這個人是個混蛋（null），就給我就找個備胎（空字串）代替。」這樣你後面的處理流程就再也不會爆炸了。
...
<!--
【核心說明】
如果你在寫 Spring 專案，別再自己在那邊 if != null 了。StringUtils.hasText 才是真男人。
...
<!--
【核心說明】
接下來是字串的搜尋功能。indexOf 就像是在全班點名。

【逐步解說】
它會告訴你那個同學坐在第幾個位置。
記得，Java 的索引是從 0 開始數的喔！
如果它回傳 -1，代表它找遍了整條街都沒看到這個字。在 Java 裡，-1 就是「我真的盡力了，但他沒來」。
-->
...
【逐步解說】
這個是「從後門開始點名」。這在處理副檔名（比如 photo.v1.jpg）時很有用，你只想知道「最後一個點」在哪裡，對吧？
-->
...
<!--
【核心說明】
剛才我們是找一個字，現在我們是找「一串字」。用法跟剛才完全一樣。
...
【逐步解說】
startsWith 檢查 URL 是不是 https:// 開頭，或是檔名是不是 .pdf 結尾時，是最好用的工具。
-->
...
【生活化比喻】
這就像是我設定了一個「篩選範本」。如果你要檢查手機號碼或 Email，matches 就是你的安檢員。但記得，它很嚴格，要整串都符合才算數。
...
<!--
【核心說明】
擷取子字串。這個 (begin, end) 的規則，我敢保證你們一定會寫錯。

【逐步解說】
它是「包含頭，不包含尾」。這就像是你去買切好的西瓜，你說要從第 1 片到第 3 片，老闆會給你第 1、2 片。為什麼？因為那是「切口」的位置啦！
-->
...
<!--
【問題引導】
這題是面試經典。不要只會算一次，要算「總共」出現幾次。你要學會怎麼一邊找、一邊跳、一邊數。這就像是玩跳格子，每踩到一個「禰豆子」，就要記一分。如果你是工程師，你要怎麼寫一段程式，讓電腦自動幫你算出她到底出現了幾次？
-->
...
<!--
【出題前的鋪陳】
剛才是算次數，現在我們要改字。如果我只想改「最後一個」出現的禰豆子，把它變成「竹筒」，該怎麼做？
注意喔，不是全部改，是只改最後一個。
-->
...
<!--
【核心說明】
如果你想一次把字串裡的所有某個字換掉，用 replace 就對了。它會把所有的目標都換掉。如果你想把「前任」全部換成「竹筒」，這招超快。
...
【逐步解說】
replaceAll 支援 Regex。比如你可以一次把字串裡所有的「髒話」或「手機號碼」都換掉。而 replaceFirst 則是很客氣，它只換第一個遇到的目標，後面的就裝作沒看到。
-->
...
<!--
【核心說明】
這就是「理髮師剪邊角」。它只會剪掉你額頭跟脖子多出來的毛（前後空白）。但如果你頭皮上有斑禿（中間空格），它是沒辦法幫你補起來的。
...
<!--
【核心說明】
strip 是 trim 的 2.0 版。它能處理更多奇怪國家的空白。身為一個現代開發者，請擁抱 strip，讓 trim 留在 1995 年吧。
...
<!--
【核心說明】
你想大笑三次？s.repeat(3)。你想畫一條長長的分隔線？"-".repeat(50)。這比寫迴圈優雅多了，程式碼讀起來甚至有點詩意。
...
<!--
【核心說明】
這題如果寫錯，你就不用去面試了。這是本章第二個五顆星重點：絕對不要用 == 比較字串內容。

【生活化比喻】
== 是比「門牌號碼」。equals 是比「家裡裝潢」。如果你跟鄰居的房子蓋得一模一樣（內容相同），但門牌不同（位址不同），== 會說你們家不一樣，但 equals 會說你們家長得一樣。懂了嗎？
-->
...
<!--
【核心說明】
就像剛才提到的 Email 範例，有時候我們不想管它是大寫還是小寫。這時候就用加了 IgnoreCase 的版本。
...
<!--
【核心說明】
要把數字變字串？別再用 + "" 這種歪門邪道了。String.valueOf 才是名門正派的寫法，它甚至能優雅地處理 null。
...
<!--
【核心說明】
這是一個非常專業的工具，讓你像在填空一樣製作字串。這就像是印「公文範本」。你先刻好一個模板，上面留一些空格（像是 %s、%d），最後再把資料一個一個塞進去。
...
<!--
【核心說明】
這是 Java 15 之後的新寵兒，是剛才 String.format 的帥氣進化版。讀起來更直覺，就像是模板自己會把自己變漂亮一樣。
...
<!--
【核心說明】
這就是字串界的「切片大師」。不管是 CSV 檔案還是空白分隔的字串，一刀下去，通通變成陣列。這是在處理資料庫匯出檔（CSV）或是一長串清單時最常用的招數。
...
<!--
【核心說明】
這是 split 的反向操作。切開之後想黏回去？String.join 就是你的強力膠。你還可以指定黏接處的圖案（分隔符號）。這在把清單轉成顯示用的文字時非常方便。
...
<!--
【核心說明】
這是一個讓程式碼變得很「絲滑」的進階技巧。這是寫出「優雅代碼」的秘訣。它讓你像在跑大隊接力一樣，字串一直傳下去處理，完全不用停下來喘口氣存變數。這就是所謂的「功能性程式設計」風格。
...
<!--
【🎯 章節標題頁】
最後，我們要來認識字串界的「變形金剛」：StringBuffer 和 StringBuilder。如果你在迴圈裡用 + 號拼字串，你的伺服器記憶體會像是在滴血。
...
<!--
【核心說明】
我們來直接看程式碼的對比。StringBuilder 就是你的專屬工具籃。不管你塞多少東西，它都還是在同一個籃子裡，不會一直去買新籃子再把舊的丟掉。

【逐步解說】
左邊的寫法，每次 s += i，Java 都會偷偷幫你做：建立一個新的 String 物件、複製內容、再指向它。做 100 次就產生 100 個垃圾物件。
而訂右邊的 StringBuilder 就像是一個「可以伸縮的籃子」。
...
<!--
【核心說明】
對 StringBuilder 來說，最重要的動作就是加字。append 是加在後面，insert 是暴力插隊。這對效能來說是極致的優化，在大數據處理時非常有感。
...
<!--
【核心說明】
倒轉字串。這招在面試考「迴文」時簡直是降維打擊。

【逐步解說】
delete 也是包含頭不包含尾的規則。
而我最喜歡的方法就是 reverse。
如果你想把字串反過來寫，以前可能要寫一個複雜的迴圈，現在讓點一下 .reverse()，它就幫你倒過來了。這在做某些演算法題目時超像在作弊！
-->
...
<!--
【核心說明】
這是一個經典的考題：StringBuffer 和 StringBuilder 到底差在哪？這是一個著名的「保全 vs 效率」的故事。大部份時間我們都選 StringBuilder，因為我們不喜歡排隊（執行緒安全），我們追求的是極致的速度！
