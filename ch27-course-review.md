...
- **基礎語法**（Ch 1–7）— 平台、變數、運算子、流程、迴圈、陣列
- **物件導向**（Ch 9–9、14–17）— 類別、封裝、繼承、多形、抽象、介面
- **常用 API**（Ch 11–13、18）— Math、日期時間、字串、正規表達式、包裝類別
- **工程實務**（Ch 20–20）— 套件、例外處理
- **現代 Java**（Ch 25–25）— 集合框架、Lambda 與 Stream
- **綜合練習** — 圖書館借閱系統
...
| 第二篇 物件導向 | Ch 9–9、14–17 | 讓程式「有結構」：類別、封裝、繼承、多形 |
| 第三篇 常用 API | Ch 11–13、18 | 站在巨人肩膀上：Math、時間、字串、Regex |
| 第四篇 工程實務 | Ch 20–20 | 讓程式「不會爆」：套件管理與例外處理 |
| 第五篇 現代 Java | Ch 25–25 | 讓程式「更精煉」：集合、Lambda、Stream |
...
【回顧指引】
細節模糊的話，回 Ch 1（平台概念）和 Ch 3（程式結構與註解）。
-->
...
⚠️ 易錯點：
String 是參照型態不是基本型態——比較兩個字串相不相等，要用 equals() 而不是 ==，這是全課程最常見的 bug 之一（Ch 16 會再深入）。

【回顧指引】
溢位、擴大/縮小轉換的細節在 Ch 4；printf 格式化輸出也在那一章。
-->
...
【回顧指引】
Scanner 輸入、parseInt/parseDouble 字串轉數值也在 Ch 5。
-->
...
【回顧指引】
巢狀迴圈（九九乘法表）、Scanner 輸入驗證、雞兔同籠專題在 Ch 7。
-->
...
⚠️ 易錯點：
b = a 之後改 b[0]，a[0] 也跟著變——因為兩個變數指向同一塊記憶體。要真正複製，用 Arrays.copyOf()。這個「參照 vs 複製」的觀念在 Ch 9 的物件上會再出現一次。

【回顧指引】
Stack/Heap 記憶體圖解、矩陣運算、線性搜尋在 Ch 8。
-->
...
【給學生的提示】
先別看下一頁，自己動手寫。卡住的點通常是「平均變成整數」——想想 Ch 5 的整數除法陷阱。
-->
...
【回顧指引】
this 關鍵字、變數範圍（Scope 遮蔽）、物件陣列在 Ch 9。
-->

---

# Ch 10 複習：建構子、封裝與 static
...
【回顧指引】
四種存取修飾詞（private / default / protected / public）比較表、JavaBean 慣例在 Ch 10。想看「不封裝會發生什麼事」的實況演示，可以看特別篇的 AI 協作試教。
-->
...
【回顧指引】
IS-A vs HAS-A、方法隱藏、向下轉型（Downcasting）與 instanceof 在 Ch 15。
-->

---

# Ch 17–17 複習：抽象類別 vs 介面
...
【回顧指引】
介面多重繼承、default 方法、實作衝突的解法在 Ch 17–17。
-->
...
【回顧指引】
Objects 工具類別（isNull、requireNonNullElse）、現代化 equals 寫法在 Ch 16。
-->
...
Circle 的 area 是 Math.PI * r * r（Ch 11 的 Math 類別馬上要複習）。
-->
...
【回顧指引】
Character 類別（isDigit、isLetter）、StringBuilder、跳脫字元在 Ch 13。
-->
...
【回顧指引】
貪婪 vs 懶惰量詞、Pattern/Matcher、Email 與身分證驗證練習在 Ch 14。
-->
...
【回顧指引】
Integer/Double/Boolean 的完整方法、MIN_VALUE/MAX_VALUE 常數在 Ch 19。
-->
...
<!--
【過場說明】
第四篇從「寫得出來」進化到「寫得穩固」：套件讓大型專案有秩序，例外處理讓程式面對錯誤不會直接死掉。
-->
...
【回顧指引】
萬用字元 import 的限制（不含子套件）、JDK 常用標準套件清單在 Ch 20。
-->
...
⚠️ 易錯點：
（1）Map 的 put 遇到相同 key 是「覆蓋」不是報錯。（2）集合的泛型只能放包裝類別：List<Integer> 不能寫 List<int>——呼應 Ch 19。（3）自訂類別放 HashSet 要 Override equals + hashCode——呼應 Ch 16。

【回顧指引】
Iterator、Map 三種遍歷方式、選用指南流程圖在 Ch 25。
-->

---

# Ch 26 複習：Lambda 與 Stream
...
【回顧指引】
四大函數式介面、Optional、Collectors.groupingBy 在 Ch 26。
-->
...
<!--
【解題重點】
mapToInt 的必要性：一般 Stream<Integer> 沒有 average()，要先轉成基本型態特化的 IntStream——這裡又用上了 Ch 19 的裝箱拆箱知識。
...
<!--
【結尾】
開放提問。常見問題方向：面試考什麼（封裝、多形、equals/hashCode、集合選用）、下一步學什麼（Spring Boot）。
